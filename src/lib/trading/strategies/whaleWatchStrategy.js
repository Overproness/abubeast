...
export class WhaleWatchStrategy {
  ...
  async analyzeToken(tokenData) {
    try {
      ...
      analysis.whaleTransactions = await this.getWhaleTransactions(
        tokenData.mint_address
      );

      // Analyze whale activity patterns
      analysis.whaleActivity = this.analyzeWhaleActivity(
  ...
  async getWhaleTransactions(mintAddress) {
    try {
      ...
      console.error(
        "[WhaleWatchStrategy] Error fetching whale transactions:",
        error
      );
      return [];
    }
  ...
  analyzeWhaleActivity(transactions) {
    ...
    const whaleTransactions = transactions.filter((tx) => {
      ...
      return (
        tx.isWhale || (tx.amount && tx.amount >= this.config.whaleThreshold)
      );
    });
    ...
    activity.buyTransactions = whaleTransactions.filter(
      (tx) => tx.type === "buy"
    );
    activity.sellTransactions = whaleTransactions.filter(
      (tx) => tx.type === "sell"
    );

    // Calculate volumes
    const buyVolume = activity.buyTransactions.reduce(
      (sum, tx) => sum + tx.amount,
      0
    );
    const sellVolume = activity.sellTransactions.reduce(
      (sum, tx) => sum + tx.amount,
      0
    );
  ...
  calculateConcentrationRisk(activity) {
    ...
    const totalVolume = activity.topWhales.reduce(
      (sum, whale) => sum + whale.volume,
      0
    );
  ...
  generateWhaleSignals(whaleActivity) {
    ...
    // Accumulation signal
    if (
      whaleActivity.activityTrend === "accumulation" ||
      whaleActivity.activityTrend === "strong_accumulation"
    ) {
      signals.accumulation = true;
      const strength =
        whaleActivity.activityTrend === "strong_accumulation" ? 0.8 : 0.4;
    ...
    // Distribution signal
    if (
      whaleActivity.activityTrend === "distribution" ||
      whaleActivity.activityTrend === "strong_distribution"
    ) {
      signals.distribution = true;
      const strength =
        whaleActivity.activityTrend === "strong_distribution" ? 0.8 : 0.4;
      signals.signalStrength -= strength;
      ...
      const strength = Math.min(
        0.6,
        Math.abs(whaleActivity.netVolume) / 100000
      );
      signals.distributionSignal = strength;
      signals.signalStrength -= strength;
    }
    ...
    if (whaleActivity.concentrationRisk > 0.5) {
      // Use fixed threshold instead of config
  ...
  generateWhaleRecommendation(analysis) {
    ...
    if (
      (whaleActivity.totalWhales > 0 ||
        whaleSignals.accumulationSignal > 0 ||
        whaleSignals.distributionSignal > 0) &&
      whaleSignals.accumulationSignal > 0 &&
      whaleSignals.distributionSignal > 0 &&
      Math.abs(
        whaleSignals.accumulationSignal - whaleSignals.distributionSignal
      ) < 0.3
    ) {
    ...
    // Distribution signals (check before accumulation to handle whale distribution scenario)
      ...
      whaleSignals.distributionSignal > whaleSignals.accumulationSignal &&
      (whaleSignals.distributionSignal > 0.2 ||
        whaleSignals.distribution ||
        whaleActivity.activityTrend === "distribution" ||
        whaleActivity.activityTrend === "strong_distribution")
    ) {
      // For strong distribution with high sell volume, recommend AVOID instead of SELL
      if (
        whaleActivity.sellVolume > whaleActivity.buyVolume * 2 ||
        whaleActivity.activityTrend === "strong_distribution"
      ) {
        ...
        const trendText =
          ...
          whaleActivity.activityTrend !== "neutral"
            ? whaleActivity.activityTrend
            : "selling pressure";
    ...
    // Strong accumulation signal
      ...
      whaleSignals.accumulationSignal > 0.3 ||
      (whaleSignals.accumulation &&
        whaleActivity.activityTrend === "strong_accumulation")
    ) {
    ...
    // Concentration risk
      ...
      recommendation.action = "AVOID";
      recommendation.reason = `High concentration risk - top whales control ${Math.round(
        whaleActivity.concentrationRisk * 100
      )}% of volume`;
      recommendation.reasoning = recommendation.reason;
    }
    ...
    return {
      ...
      parameters: {
        ...
        concentrationRiskThreshold:
          TRADING_CONFIG.SOCIAL_SENTIMENT.WHALE_SENTIMENT
            .concentrationRiskThreshold,
      },
