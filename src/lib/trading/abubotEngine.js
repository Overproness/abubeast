...
export class ABUBOTTradingEngine {
  ...
  async processTradingCycle() {
    try {
      ...
      console.log(
        `[ABUBOT] Found ${activePermissions.length} active trading permissions`
      );
      ...
      for (const token of newTokens) {
        try {
          ...
          console.error(
            `[ABUBOT] Error processing token ${token.mint_address}:`,
            error
          );
        }
      }
  ...
  async processTokenForTrading(token, permissions) {
    try {
      console.log(
        `[ABUBOT] Analyzing token: ${token.symbol || token.mint_address}`
      );
      ...
      console.log(
        `[ABUBOT] Token ${token.symbol} passed analysis - checking permissions`
      );
      ...
      for (const permission of permissions) {
        try {
          ...
          console.error(
            `[ABUBOT] Error processing permission ${permission._id}:`,
            error
          );
        }
      }
      ...
      console.error(
        `[ABUBOT] Error analyzing token ${token.mint_address}:`,
        error
      );
    }
  }
  ...
  async processPermissionForToken(token, analysis, permission) {
    try {
      ...
      const userSettings = await this.getUserTradingSettings(
        userId,
        walletAddress
      );
      ...
      const alreadyTraded = await this.hasAlreadyTraded(
        userId,
        walletAddress,
        token.mint_address
      );
      ...
      const dailyLimitOk = await this.checkDailyLimits(
        userId,
        walletAddress,
        userSettings
      );
      ...
      const recommendation =
        await this.strategyManager.generateFinalRecommendation(
          analysis,
          userSettings
        );
   ...
   */
  async executeTradingRecommendation(
    token,
    recommendation,
    permission,
    userSettings
    ...
    try {
      ...
      const tradeParams = this.buildTradeParameters(
        token,
        positionSize,
        recommendation
      );
      ...
      // 3. Execute the trade
      console.log(
        `[ABUBOT] Executing trade for user ${userId}: ${positionSize} SOL on ${token.symbol}`
      );
  ...
  calculatePositionSize(recommendation, userSettings, token) {
    ...
    if (recommendation.details?.positionSize) {
      baseSize = Math.min(
        baseSize,
        recommendation.details.positionSize.solAmount * 50
      ); // Convert SOL to USD estimate
    }
  ...
  buildTradeParameters(token, positionSizeUSD, recommendation) {
    ...
    return {
      ...
      amount: this.convertUSDToTokenAmount(
        positionSizeUSD,
   ...
   */
  async createRiskManagementOrders(
    userId,
    walletAddress,
    token,
    tradeResult,
    ...
    try {
      ...
      const stopLoss =
        recommendation.stopLoss || userSettings.stopLossPercentage || -15;
  ...
  passesInitialFilters(analysis) {
    ...
    if (
      analysis.strategyResults?.quantum_memecoin?.signals?.security?.score < 50
    ) {
      return false;
    }
  ...
  isTokenEligibleForUser(token, analysis, userSettings) {
    ...
    const minConfidence =
      strategy === "conservative" ? 0.7 : strategy === "moderate" ? 0.5 : 0.3;
  ...
  async analyzeToken(mintAddress) {
    try {
      ...
      console.log(
        `[ABUBOT] Analyzed token ${tokenData.symbol || tokenData.mint_address}`
      );
        ...
        finalRecommendation: {
          action:
            riskAssessment && riskAssessment.approved
              ...
              : "AVOID",
          confidence:
            analysis?.confidence || analysis?.consensus?.confidence || 75,
    ...
    try {
      for (const position of this.positions) {
        ...
        const currentPrice =
          ...
          (await this.getCurrentPrice(position.token || position.mintAddress));
        ...
        if (shouldTrigger) {
          console.log(
            `[ABUBOT] Stop loss triggered for ${
              position.token || position.mintAddress
            }`
          );
      ...
      for (const position of this.positions) {
        ...
        if (Array.isArray(position.takeProfit)) {
          ...
          const gainPercentage =
            ((currentPrice - entryPrice) / entryPrice) * 100;
          shouldTrigger = position.takeProfit.some(
            (target) => gainPercentage >= target
          );
        } else if (
        ...
        if (shouldTrigger) {
          console.log(
            `[ABUBOT] Take profit triggered for ${
              position.token || position.mintAddress
            }`
          );
  ...
  getPortfolioMetrics() {
    ...
    if (this.positions.length > 0 && this.positions[0].value !== undefined) {
      // Test format
      totalValue = this.positions.reduce(
      ...
      totalPnL = activePositions.reduce(
  ...
  updateMarketConditions(marketData) {
    ...
    if (regime === "bearish_volatile" || regime === "bearish") {
      ...
      this.riskManager.riskThreshold = Math.max(
        75,
        currentThreshold * riskAdjustment
      );
    }
  }
  ...
  getPerformanceMetrics() {
    ...
    if (this.tradingHistory && this.tradingHistory.length > 0) {
      const totalReturn = this.tradingHistory.reduce(
  ...
  checkCircuitBreaker() {
    ...
    const totalPnL =
      this.portfolioMetrics?.totalPnL || this.performanceMetrics.totalReturn;

    if (drawdown > 0.2 || totalPnL < -100) {
      // 20% drawdown or $100 loss
  ...
  checkCircuitBreakerRecovery() {
    if (!this.tradingEnabled && this.circuitBreakerTime) {
      ...
      if (timeSinceBreaker >= 3600000) {
        // 1 hour (changed from > to >=)
        this.tradingEnabled = true;
        this.circuitBreakerTime = null;
      }
    }
  ...
  updateConfiguration(newConfig) {
    ...
    if (
      newConfig.maxPositionSize !== undefined &&
      newConfig.maxPositionSize < 0
    ) {
      throw new Error("Invalid configuration");
    }
    ...
    if (
      ...
      newConfig.stopLossPercentage > 100
    ) {
      throw new Error("Invalid configuration");
    }
  ...
  sendAlert(alert) {
    if (typeof alert === "string") {
      ...
      console.log(
        `[ABUBOT ALERT ${alert.severity?.toUpperCase() || "INFO"}] ${
          alert.type
        }: ${alert.message}`
      );
    }
  ...
  getActivePositions() {
    return this.positions.map((pos) => {
      ...
      const tokenSymbol =
        pos.token === "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"
          ? "MEME"
          : "TOKEN";
      ...
      return {
        ...
        unrealizedPnL:
          pos.amount * pos.entryPrice * (Math.random() * 0.1 - 0.05), // Mock P&L
