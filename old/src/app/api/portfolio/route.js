...
export async function GET(request) {
  try {
    ...
    // Verify user authentication (optional - allow public access for portfolio viewing)
    ...
    if (token) {
      try {
        ...
        isAuthenticated = tokenData && tokenData.id;
        console.log("[Portfolio API] Token verification result:", isAuthenticated);
      } catch (error) {
        console.log("[Portfolio API] Token verification failed:", error);
...
async function fetchSolanaPortfolio(walletAddress) {
  try {
    ...
    const solPrice = tokenPrices.solana?.usd || 100; // Fallback price if API fails
...
async function getTokenPrices(tokenIds) {
  try {
    ...
    const cacheKey = idsString;
    
    // Check cache first
    const cached = priceCache.get(cacheKey);
    if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
      console.log("[Portfolio API] Using cached prices");
      return cached.data;
    }
    ...
    if (!response.ok) {
      console.warn(`[Portfolio API] Price API returned ${response.status}, using fallback prices`);
...
// Fallback prices for common tokens when API is rate limited
function getFallbackPrices(tokenIds) {
  ...
  console.log("[Portfolio API] Using fallback prices:", result);
  return result;
}
