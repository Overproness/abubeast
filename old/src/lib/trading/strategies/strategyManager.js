...
export class StrategyManager {
    ...
    try {
      ...
      for (const [name, strategy] of this.strategies) {
        try {
          ...
          console.error(
            `[StrategyManager] Failed to initialize ${name}:`,
            error
          );
        }
      }
  ...
  async analyzeToken(tokenData, userSettings = {}) {
    try {
      ...
      const strategyPromises = Array.from(this.activeStrategies).map(
        async (strategyName) => {
          try {
            ...
            console.error(
              `[StrategyManager] Strategy ${strategyName} analysis failed:`,
              error
            );
            return {
              strategyName,
      ...
      strategyResults.forEach(({ strategyName, result }) => {
        ...
        this.strategyResults.set(
          `${tokenData.mint_address}_${strategyName}`,
          result
        );
      });
      ...
      if (this.riskManager) {
        try {
          analysis.riskAssessment = await this.riskManager.assessRisk(
            tokenData
          );
          ...
          if (analysis.riskAssessment && !analysis.riskAssessment.approved) {
            ...
            analysis.consensus.confidence = Math.min(
              analysis.consensus.confidence,
              0.3
            );
            analysis.consensus.reasoning =
              "Risk assessment override: High risk detected";
          }
        } catch (error) {
      ...
      analysis.recommendation = this.generateFinalRecommendation(
        analysis,
        userSettings
      );
  ...
  calculateConsensus(strategyResults) {
    ...
    const consensusConfidence =
      totalWeight > 0
        ? confidences.reduce((sum, conf) => sum + conf, 0) / totalWeight
        : 0;
  ...
  generateFinalRecommendation(analysis, userSettings) {
    ...
    let recommendation = {
      ...
      reason: `Consensus: ${consensus.action} (${Math.round(
    ...
    recommendation.details = this.extractStrategyDetails(
  ...
  applyConservativeFilter(recommendation, analysis) {
    ...
    const avgSecurityScore =
      securityScores.length > 0
        ? securityScores.reduce((sum, score) => sum + score, 0) /
  ...
  applyAggressiveFilter(recommendation, analysis) {
    ...
    if (aggressive.consensus.strength >= 0.4 && aggressive.action === "HOLD") {
      ...
      const topRecommendations = topStrategies
        .map(
          (strategyName) =>
  ...
  extractStrategyDetails(strategyResults) {
    ...
    Object.values(strategyResults).forEach((result) => {
      ...
      if (result.riskAssessment?.riskScore) {
        details.riskFactors.push(
          `Risk Score: ${result.riskAssessment.riskScore}`
        );
      }
    });
  ...
  getEnabledStrategies() {
    ...
    return Array.from(this.activeStrategies).map(
      (name) => reverseAliases[name] || name
    );
  }
}
